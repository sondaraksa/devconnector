...
router.get('/:id',(req, res) => {
    Post.findById(req.params.id)
        ...
        .catch(err => res.status(404).json({nopostfound: 'No Post found with at id'}));
})
...
router.post('/',passport.authenticate('jwt',{ session: false }),(req, res) => {
    ...
    //check validation
    if(!isValid){
        //if there is any errors send object
    ...
    Profile.findOne({ user: req.user.id })
        .then(profile => {
            ...
            .then(post => {
                ...
                //delete
                post.remove().then(()=> res.json({ seccuss: true }));
            })
...
router.post('/unlike/:id',passport.authenticate('jwt',{session: false}),(req,res)=>{
    Profile.findOne({ user: req.user.id })
        .then(profile => {
            ...
            .then(post => {
                if(post.likes.filter(like => like.user.toString() === req.user.id).lenght === 0){
                    return res.status(400).json({ alreadyliked: 'You have not yet likedthis post'});
                }
